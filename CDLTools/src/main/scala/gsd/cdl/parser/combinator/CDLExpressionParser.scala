package gsd.cdl.parser.combinator
...
trait CDLExpressionParser extends JavaTokenParsers with ImplicitConversions {
  ...
  val integer : Parser[LongIntLiteral] =
      "(-)?[0-9]+".r ^^ { case i =>
        try {
          LongIntLiteral(i.toLong)
        }
        catch {
          case e: NumberFormatException =>
//            println("WARNING: " + e.getMessage + ", rounding to " + Integer.MAX_VALUE)
            LongIntLiteral( Long MaxValue )
        }
      }
    ...
    identifier ~ ("(" ~> opt(funcExpr ~ rep("," ~> funcExpr)) <~ ")") ^^
      {
        ...
        case res => sys.error("Unexpected result: " + res)
      } | factorExpr
...
object CDLExpressionParser extends CDLExpressionParser {
